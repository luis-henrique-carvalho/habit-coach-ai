...
Cypress.Commands.add(
  "createHabit",
  (name: string, options: CreateHabitOptions = {}) => {
    const {
      description,
    ...
    cy.contains("button", "Novo Hábito").click({ force: true });
    ...
    cy.get('[role="dialog"]')
      .find("#name")
      .focus()
      .type("{selectall}{backspace}" + name, { delay: 50 });
    ...
    if (description) {
      cy.get('[role="dialog"]')
        .find("#description")
        .focus()
        .type("{selectall}{backspace}" + description, { delay: 50 });
    }
    ...
    if (recurrenceType !== "daily") {
      ...
      cy.get('[role="option"]')
        .contains(recurrenceLabels[recurrenceType])
        .click();
    }
