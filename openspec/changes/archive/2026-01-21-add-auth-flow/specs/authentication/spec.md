...
- On success: The system MUST create a user, generate verification token, and send verification email.
- On error: The system MUST display field-specific validation errors.
- After successful registration: The system SHALL redirect to verification page.
...
- The system SHALL allow users to sign in with registered email and password.
- The login form MUST validate email format and require password.
- On submit: The system SHALL verify email exists and password matches.
- Invalid credentials: The system MUST show generic error (security best practice).
- On success: The system MUST create session token, set httpOnly cookie, and redirect to dashboard.
- After sign-in: The user SHALL access all protected routes.
- Session duration: MUST be configurable (default 24 hours).
...
- The system SHALL allow users to sign in using Google or GitHub accounts.
- OAuth buttons MUST be present on login and register pages.
- Providers: Google (via Google OAuth 2.0), GitHub (via GitHub OAuth 2.0)
- First OAuth sign-in: The system MUST create user account automatically.
  - Name: Extracted from OAuth profile (given_name + family_name or username)
  - Email: From OAuth provider
  - emailVerified: true (trusted provider)
- Existing email + OAuth: The system SHALL link OAuth provider to existing account.
- Session MUST be created immediately after OAuth success.
- No additional email verification needed for OAuth users.
- The system SHALL redirect to /dashboard after successful OAuth sign-in.
...
- The system SHALL create, validate, and revoke user sessions securely.
- Session MUST be stored in `session` table with:
  - userId reference
...
- Session token MUST be stored in httpOnly cookie (name: `auth.session`).
- Secure flag MUST be enabled in production (HTTPS only).
- SameSite=Strict MUST be set to prevent CSRF.
- Session middleware SHALL validate on protected routes.
- Expired sessions: MUST be automatically cleaned up by database or middleware.
- Sign-out: The system MUST delete session record and clear cookie.
...
### Requirement: Protected Routes and Middleware

- Protected routes SHALL require authentication.
- Middleware MUST check every request to `/dashboard/*`, `/habits/*`, `/goals/*`.
- If no valid session: The system MUST redirect to `/login?next={original-path}`.
- Public routes MUST be excluded from middleware check:
  - `/` (home page)
  - `/login`, `/register` (auth pages)
  - `/api/auth/*` (OAuth callbacks)
  - `/pricing` (public)
- Authenticated users SHALL be redirected from `/login` and `/register` to `/dashboard`.
- 404 handling: Auth pages MUST not be visible to authenticated users.
...
- All form submissions SHALL be validated with clear, user-friendly error messages.
- Client-side validation MUST use React Hook Form with real-time feedback.
- Server-side validation MUST be handled by Better Auth internally.
...
- Error messages MUST be in Portuguese (pt-BR).
- Field-level errors MUST show inline under input.
- Form-level errors from Better Auth MUST be shown in Alert component (via onError callback).
- Submit button MUST be disabled during request (via onRequest callback).
- Loading state MUST be shown during submission.
- Success messages SHALL be shown after certain actions (via onSuccess callback).
...
### Requirement: User Account Creation and Initialization

- The system SHALL create new user accounts with proper defaults.
- User record MUST be created in `user` table with:
...
- Account linking: The system MUST create `account` table record for OAuth.
- No initial habits/goals SHALL be created (separate user onboarding flow).
- Email verification workflow MUST be initiated for email+password users.
